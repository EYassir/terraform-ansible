...
resource "aws_lb" "tf_lb_wp" {
  name               = "tf-lb-wordpress"
  load_balancer_type = "application"
  subnets            = data.aws_subnet_ids.custom_vpc_subnets_ids.ids
...
resource "aws_lb_listener_rule" "tf_asg_rule" {
  ...
  action {
    type             = "forward"
    target_group_arn = var.target_group_arn #data.terraform_remote_state.asg.outputs.asg_taget_group.arn
  }
}
